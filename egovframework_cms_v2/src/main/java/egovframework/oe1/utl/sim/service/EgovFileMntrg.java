...
/**
 * Class Name : EgovFileMntrg.java Description : 시스템
 * 네트워크 정보를 확인하여 제공하는 Business class
...
public class EgovFileMntrg extends Thread {
    ...
    /**
     * <p>
     * 해당 파일의 변경 유무를 체크하기 위한 Default 초의 stati final 변수,
     * 기본 적용은 값은 60초
     * </p>
     */
     ...
     * <p>
     * 해당 파일의 변경 유무를 체크하기 위한 Default 초의 stati final 변수,
     * 기본 적용은 값은 60초{@link #DEFAULT_DELAY}.
     * </p>
     */
    ...
    ArrayList realOriginalList = new ArrayList(); // 최초의
                                                  // 원본리스트
    ArrayList originalList = new ArrayList(); // 직전리스트는
                                              // 주기적으로
                                              // 직전목록정보로
                                              // 갱신된다.
    ArrayList currentList = new ArrayList(); // 직전리스트와
                                             // 비교할 현시점
                                             // 리스트
    ArrayList changedList = new ArrayList(); // 직전리스트와
                                             // 비교한 시점에
                                             // 발생된
                                             // 변경리스트
    ArrayList totalChangedList = new ArrayList(); // 최초리스트와
                                                  // 비교한
                                                  // 변경
                                                  // 리스트
    // totalChangedList는 필요시 checkAndConfigure함수 내에서
    // 주석해제후 사용(부하량을 고려하여 사용)
    int cnt = 0;
    ...
    protected void doOnChange(ArrayList changedList) {
        ...
        changedList.clear(); // 직전리스트와 비교해서 변경된 내역은
                             // 로그처리한 후 초기화한다.
        originalList = new ArrayList(currentList); // 현재리스트가
                                                   // 직전리스트가
                                                   // 된다.(새로
                                                   // 생성해야
                                                   // 함!)
	...
	protected void checkAndConfigure() {
        ...
        try {
             ...
             * i++ ){//log.debug(
             * "in checkAndConfigure() ::: originalList:"
             * + originalList.get(i)); } for(int i = 0;
             * i<currentList.size(); i++ ){
             * //log.debug(
             * "in checkAndConfigure() ::: currentList:"
             * + currentList.get(i)); }
             */
            ...
            // 현재하위디렉토리정보와 초최하위디렉토리 정보를 비교한다. 삭제된 경우를
            // 확인함
            for (int i = 0; i < originalList.size(); i++) {
                for (int j = 0; j < currentList.size(); j++) {
                    str1 = (String) originalList.get(i);
                    str2 = (String) currentList.get(j);
                    if (str1.substring(0, str1.indexOf("$")).equals(
            ...
            // 현재하위디렉토리 정보와 최초하위디렉토리 정보를 비교한다.(신규로
            // 생성되었거나 수정된 경우를 확인함)
            for (int i = 0; i < currentList.size(); i++) {
                for (int j = 0; j < originalList.size(); j++) {
                    if (((String) currentList.get(i))
                        .equals((String) originalList.get(j))) {
                        isSame = true;
                    }
            ...
            // interrupted = true; // there is no point
            // in continuing
            // e.printStackTrace();
        ...
        if (changedList.size() > 0) {
            // log.debug("change occur , changed file check count:"+cnt+
            // " , changed file count:"+changedList.size());
     ...
     * </pre>
     * @return boolean isEnd 감시종료여부 중단하려면 true 리턴,
     *         계속하려면 false 리턴
     */
