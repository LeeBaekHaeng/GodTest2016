			...
			if(isRemotelyAuthenticated != null && isRemotelyAuthenticated.equals("true")){
				try{
					...
					if(loginVO != null && loginVO.getId() != null && !loginVO.getId().equals("")){
						...
						//스프링 시큐리티 로그인
						AbstractAuthenticationProcessingFilter springSecurity = null;

						@SuppressWarnings("rawtypes")
						Map beans = act.getBeansOfType(AbstractAuthenticationProcessingFilter.class);
						 if (beans.size() > 0) {
					        	springSecurity = (AbstractAuthenticationProcessingFilter)beans.values().toArray()[0];
					        } else {
