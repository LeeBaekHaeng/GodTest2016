...
<script type="text/javascript">
	...
	function fn_egov_deleteFile(atchFileId, fileSn) {
		if(confirm("저장여부와 관계없이 파일이 삭제됩니다.\n또한 임시로 저장대기중에 있는 파일도 삭제됩니다.\n삭제하시겠습니까?"))
		{
			...
			form.action = "<c:url value='/com/cmm/fms/deleteFileInfs.do'/>";
			form.submit();
		}else{
			return;
		}
		...
		<tr>
			...
			<c:choose>
				<c:when test="${updateFlag=='Y'}">
					<span id="${fileVO.orignlFileNm}"><c:out value="${fileVO.orignlFileNm}"/></span>&nbsp;[<c:out value="${fileVO.fileMg}"/>&nbsp;byte]
				...
				<c:otherwise>
					<a title="파일다운로드" href="javascript:fn_egov_downFile('<c:out value="${fileVO.atchFileId}"/>','<c:out value="${fileVO.fileSn}"/>')">
